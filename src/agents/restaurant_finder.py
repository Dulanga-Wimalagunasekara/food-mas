...
def run_restaurant_finder(state: GraphState) -> dict:
    ...
    try:
        ...
        # LLM ranking failed — use rating as fallback score
        max_rating = max((c.rating for c in raw_candidates), default=5.0)
        score_map = {c.id: c.rating / max_rating for c in raw_candidates}
    ...
    trace.write({"node": "restaurant_finder", "status": "ok", "candidates": [c.model_dump() for c in scored], "latency_ms": latency_ms})
